...
@click.option(
    "--model", "-m", default="gemini-3-pro-preview", help="Gemini model to use"
)
@click.option("--sandbox", "-s", is_flag=True, help="Run in sandbox mode")
@click.option(
    "--show-prompts", is_flag=True, help="Show input prompts and raw responses"
)
@click.pass_context
def cli(ctx, config, verbose, debug, json, no_color, model, sandbox, show_prompts):
    ...
    ctx.obj["formatter"] = OutputFormatter(use_color=not no_color, json_output=json)

    # Enable debug mode if requested
    if debug:
        ctx.obj["formatter"].info("Debug mode enabled")
